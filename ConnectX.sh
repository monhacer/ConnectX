...
check_installed_status(){
    [[ ! -e ${file} ]] && echo -e "${Error} ConnectX haven't been installed, please check it!" && exit 1
    [[ ! -e ${conf} ]] && echo -e "${Error} ConnectX config doesn't exist, please check it!" && [[ $1 != "un" ]] && exit 1
}
...
Download_ocserv(){
    ...
    [[ ! -s "ocserv-${ocserv_ver}.tar.xz" ]] && echo -e "${Error} ConnectX source download failed!" && rm -rf "ocserv/" && rm -rf "ocserv-${ocserv_ver}.tar.xz" && exit 1
    ...
    if [[ -e ${file} ]]; then
        mkdir "${conf_file}"
        wget --no-check-certificate -N -P "${conf_file}" "https://raw.githubusercontent.com/monhacer/ConnectX/main/ocserv.conf"
        [[ ! -s "${conf}" ]] && echo -e "${Error} ConnectX config download failed!" && rm -rf "${conf_file}" && exit 1
    else
        echo -e "${Error} ConnectX compiled failed!" && exit 1
    fi
}
Service_ocserv(){
    if ! wget --no-check-certificate https://raw.githubusercontent.com/ToyoDAdoubiBackup/doubi/master/service/ocserv_debian -O /etc/init.d/ocserv; then
        echo -e "${Error} ConnectX service management script downloadf failed!" && over
    fi
    chmod +x /etc/init.d/ocserv
    update-rc.d -f ocserv defaults
    echo -e "${Info} ConnectX service management script download successfully."
}
rand(){
    min=10000
    max=$((60000-$min+1))
...
Install_ocserv(){
    check_root
    [[ -e ${file} ]] && echo -e "${Error} ConnectX is already installed !" && exit 1
...
Start_ocserv(){
    check_installed_status
    check_pid
    [[ ! -z ${PID} ]] && echo -e "${Error} ConnectX is running !" && exit 1
...
Stop_ocserv(){
    check_installed_status
    check_pid
    [[ -z ${PID} ]] && echo -e "${Error} ConnectX is NOT running !" && exit 1
...
Set_ocserv(){
    [[ ! -e ${conf} ]] && echo -e "${Error} ConnectX config file doesn't exist !" && exit 1
...
List_User(){
    [[ ! -e ${passwd_file} ]] && echo -e "${Error} ConnectX account config file doesn't exist !" && exit 1
...
View_Log(){
    [[ ! -e ${log_file} ]] && echo -e "${Error} ConnectX log doesn't exist !" && exit 1
...
Uninstall_ocserv(){
    ...
    echo "Are you sure uninstall ConnectX ? (y/N)"
    echo
    ...
    if [[ ${unyn} == [Yy] ]]; then
        ...
        rm -f ocpasswd.8
        rm -f occtl.8
        echo && echo "ConnectX uninstall completed !" && echo
    else
...
over(){
    ...
    rm -f ocpasswd.8
    rm -f occtl.8
    echo && echo "install failed，ConnectX uninstalled !" && echo
}
Add_iptables(){
...
————————————
 ${Green_font_prefix}1.${Font_color_suffix} install ConnectX
 ${Green_font_prefix}2.${Font_color_suffix} uninstall ConnectX
————————————
 ${Green_font_prefix}3.${Font_color_suffix} Start ConnectX
 ${Green_font_prefix}4.${Font_color_suffix} Stop ConnectX
 ${Green_font_prefix}5.${Font_color_suffix} Restart ConnectX
————————————
...
echo
read -e -p " Enter your choice [0-9]:" num
case "$num" in
    0)
    Update_Shell
    ;;
